/**
 * @file    bench_bignum_common_mt.c
 * @author  git@bayborodov.com
 * @version 1.0.0
 * @date    17.06.2026
 *
 * @brief   Бенчмарк ядра bignum (multi-threaded).
 *
 * Использование:
 *   ./bench_bignum_common_mt <REPORT_NAME>
 *
 * Формат отчёта совместим со стилем perf-report:
 *   benchmarks/reports/<REPORT_NAME>_mt.txt
 */
...
static void *thread_main(void *p)
{
    ...
    uint64_t seed = (uint64_t)(a->thread_id + 1) * 0x9E3779B97F4A7C15ULL;

    bench_result_t r = {0};
    r.label          = variant_name(a->variant);
    r.variant        = a->variant;
    r.warmup_iters   = WARMUP_ITERS;
    r.measure_iters  = MEASURE_ITERS;
    r.total_iters    = WARMUP_ITERS + MEASURE_ITERS;
    ...
    for (uint64_t i = 0; i < WARMUP_ITERS; ++i) {
        dirty_buffer(&b, a->variant, seed + i);
        bignum_normalize(&b);
    }
    ...
    r.ns_per_op   = (double)dt / (double)MEASURE_ITERS;
    r.ops_per_sec = (double)MEASURE_ITERS * 1e9 / (double)dt;
    r.bytes_per_sec = (double)(BIGNUM_CAPACITY * sizeof(uint64_t))
                    * (double)MEASURE_ITERS * 1e9 / (double)dt;

    a->result = r;
    return NULL;
}

static bench_result_t bench_normalize_mt(variant_t v, int n_threads)
{
    pthread_t       *threads = calloc((size_t)n_threads, sizeof(pthread_t));
    thread_arg_t    *args    = calloc((size_t)n_threads, sizeof(thread_arg_t));
    ...
    agg.label          = variant_name(v);
    agg.variant        = v;
    agg.warmup_iters   = WARMUP_ITERS;
    agg.measure_iters  = MEASURE_ITERS * (uint64_t)n_threads;
    agg.total_iters    = (WARMUP_ITERS + MEASURE_ITERS) * (uint64_t)n_threads;

    double sum_ns = 0.0;
    for (int i = 0; i < n_threads; ++i) {
        sum_ns      += args[i].result.ns_per_op;
        agg.ops_per_sec += args[i].result.ops_per_sec;
        agg.bytes_per_sec += args[i].result.bytes_per_sec;
    }
    agg.ns_per_op = sum_ns / (double)n_threads;
...
int main(int argc, char **argv)
{
    ...
    snprintf(path, sizeof(path),
             "benchmarks/reports/%s_mt.txt", report_name);
    ...
    print_header(fp, "mt", report_name);

    for (int v = 0; v < (int)VAR__COUNT; ++v) {
        bench_result_t r = bench_normalize_mt((variant_t)v, n_threads);
        print_row(fp, &r);
        printf("%-20s %8.2f ns/op  (T=%d)\n",
               r.label, r.ns_per_op, n_threads);
    }
    print_footer(fp);
    fclose(fp);
