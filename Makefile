...
install: clean $(OBJ) $(OBJECTS) | $(DIST_INCLUDE_DIR) $(DIST_LIB_DIR)
	...
	@cp $(INCLUDE_DIR)/$(FAMILY_NAME).h $(HEADER) $(foreach dir,$(SUBMODULES_INCLUDE_DIR),$(wildcard $(dir)/*.h)) $(DIST_INCLUDE_DIR)/
...
# 4.3. Вставляем содержимое $(LIB_NAME).h, но БЕЗ его include guards и БЕЗ #include "bignum.h"
	...
	@sed '/BIGNUM_H/d' $(INCLUDE_DIR)/$(FAMILY_NAME).h >> $(SINGLE_HEADER)
	@echo "" >> $(SINGLE_HEADER)
