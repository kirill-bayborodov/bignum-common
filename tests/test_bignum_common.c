...
static void test_normalize(void)
{
    ...
    /* ------------------------------------------------------------------ */
    /* Тест на баг 1: обнуление хвоста за пределами старого len           */
    /* ------------------------------------------------------------------ */
    bignum_init(&b);
    // Заполняем весь массив "мусором"
    for (size_t i = 0; i < BIGNUM_CAPACITY; ++i) {
        b.words[i] = 0xBADBADBADBADBADBULL;
    }
    // Имитируем ситуацию: старый len = 10, но старшие 4 слова обнулились
    b.len = 10;
    b.words[5] = 0x12345678; // Новое старшее слово (индекс 5 -> len будет 6)
    b.words[6] = 0;
    b.words[7] = 0;
    b.words[8] = 0;
    b.words[9] = 0;
    ...
    ASSERT(b.len == 6, "normalize: len должен корректно обновиться до 6");
    
    // Проверяем, что слова с 6 по 31 обнулены (включая старый мусор с 10 по 31)
    int tail_clean = 1;
    for (size_t i = 6; i < BIGNUM_CAPACITY; ++i) {
        if (b.words[i] != 0) {
            tail_clean = 0;
            break;
        }
    }
    ASSERT(tail_clean == 1, "normalize: мусор в хвосте (за пределами старого len) должен быть обнулен");
    OK("normalize — очистка мусора за пределами старого len");

    /* ------------------------------------------------------------------ */
    /* Тест на баг 2: защита от мусорного len > BIGNUM_CAPACITY           */
    /* ------------------------------------------------------------------ */
    bignum_init(&b);
    b.words[0] = 0x42;
    b.len = BIGNUM_CAPACITY + 100; // Мусорное значение len
    
    // Если нет защиты, здесь произойдет Segmentation Fault из-за чтения вне буфера
    bignum_normalize(&b);
    
    ASSERT(b.len == 1, "normalize: должен пережить мусорный len и найти реальный");
    ASSERT(b.words[0] == 0x42, "normalize: данные не должны повредиться");
    OK("normalize — защита от len > CAPACITY");
